...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.clients.update", [$client->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="documents">{{ trans('cruds.client.fields.document') }}</label>
                ...
                <select class="form-control select2 {{ $errors->has('documents') ? 'is-invalid' : '' }}" name="documents[]" id="documents" multiple>
                    @foreach($documents as $id => $document)
                        <option value="{{ $id }}" {{ (in_array($id, old('documents', [])) || $client->documents->contains($id)) ? 'selected' : '' }}>{{ $document }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.client.fields.document_helper') }}</span>
            </div>
            <div class="form-group">
