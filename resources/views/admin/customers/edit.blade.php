...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.customers.update", [$customer->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="role_id">{{ trans('cruds.customer.fields.role') }}</label>
                <select class="form-control select2 {{ $errors->has('role') ? 'is-invalid' : '' }}" name="role_id" id="role_id" required>
                    @foreach($roles as $id => $entry)
                        <option value="{{ $id }}" {{ (old('role_id') ? old('role_id') : $customer->role->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('role'))
                    <div class="invalid-feedback">
                ...
                @endif
                <span class="help-block">{{ trans('cruds.customer.fields.role_helper') }}</span>
            </div>
            <div class="form-group">
